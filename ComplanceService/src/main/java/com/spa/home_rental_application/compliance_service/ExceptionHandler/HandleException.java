package com.spa.home_rental_application.compliance_service.ExceptionHandler;

import com.spa.home_rental_application.compliance_service.Exceptionclass.DTO.APIErrorResponse;
...
public class HandleException {
    ...
    @ExceptionHandler(ReraNotFoundException.class)
    public ResponseEntity<APIErrorResponse> handleReraNotFound(ReraNotFoundException ex, HttpServletRequest req) {
        log.warn("RERA not found at {}: {}", req.getRequestURI(), ex.getMessage());
        return build(HttpStatus.NOT_FOUND, ex.getMessage(), ex.getErrorCode(), req);
    }

    @ExceptionHandler(ReraAlreadyRegisteredException.class)
    public ResponseEntity<APIErrorResponse> handleReraDup(ReraAlreadyRegisteredException ex, HttpServletRequest req) {
        return build(HttpStatus.CONFLICT, ex.getMessage(), ex.getErrorCode(), req);
    }

    @ExceptionHandler(GstInvoiceNotFoundException.class)
    public ResponseEntity<APIErrorResponse> handleInvoiceMissing(GstInvoiceNotFoundException ex, HttpServletRequest req) {
        return build(HttpStatus.NOT_FOUND, ex.getMessage(), ex.getErrorCode(), req);
    }

    @ExceptionHandler(InvoiceAlreadyExistsException.class)
    public ResponseEntity<APIErrorResponse> handleInvoiceDup(InvoiceAlreadyExistsException ex, HttpServletRequest req) {
        return build(HttpStatus.CONFLICT, ex.getMessage(), ex.getErrorCode(), req);
    }

    @ExceptionHandler(PdfGenerationException.class)
    public ResponseEntity<APIErrorResponse> handlePdf(PdfGenerationException ex, HttpServletRequest req) {
        log.error("PDF generation failed at {}: {}", req.getRequestURI(), ex.getMessage(), ex);
