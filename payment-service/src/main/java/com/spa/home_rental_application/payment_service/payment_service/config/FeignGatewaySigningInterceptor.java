...
/**
 * Outbound Feign HMAC signing — ported from auth-service /
 * property-service. Without this bean payment-service's Feign calls
 * to {@code UserClient} / {@code PropertyClient} hit downstream
 * {@code GatewayAuthFilter} with no signing headers and get
 * rejected as {@code 403 GATEWAY_REQUIRED}.
 *
 * <p>Each service keeps its own copy of this class because
 * auth-commons doesn't have a Feign dependency on its compile
 * classpath, and adding one would force the reactive gateway to
 * pull in servlet-Feign too.
 */
@Configuration
@Slf4j
