package com.spa.home_rental_application.kyc_service.ExceptionHandler;

import com.spa.home_rental_application.kyc_service.Exceptionclass.DTO.APIErrorResponse;
...
/**
 * Single error envelope for KYC Service. Mirrors the structure used by
 * user-service / property-service so the frontend can reuse one parser.
 */
...
public class HandleException {
    ...
    public ResponseEntity<APIErrorResponse> handleKycNotFound(KycNotFoundException ex, HttpServletRequest req) {
        log.warn("KYC not found at {}: {}", req.getRequestURI(), ex.getMessage());
        return build(HttpStatus.NOT_FOUND, ex.getMessage(), ex.getErrorCode(), req);
    }

    @ExceptionHandler(KycAlreadyVerifiedException.class)
    public ResponseEntity<APIErrorResponse> handleAlreadyVerified(KycAlreadyVerifiedException ex, HttpServletRequest req) {
        return build(HttpStatus.CONFLICT, ex.getMessage(), ex.getErrorCode(), req);
    }

    @ExceptionHandler(InvalidKycDataException.class)
    public ResponseEntity<APIErrorResponse> handleInvalidKyc(InvalidKycDataException ex, HttpServletRequest req) {
        return build(HttpStatus.BAD_REQUEST, ex.getMessage(), ex.getErrorCode(), req);
    }

    @ExceptionHandler(KycProviderException.class)
    public ResponseEntity<APIErrorResponse> handleProviderFail(KycProviderException ex, HttpServletRequest req) {
        log.error("KYC provider failure at {}: {}", req.getRequestURI(), ex.getMessage(), ex);
