...
login_user() {
  local username=$1
  local resp
  # Backend LoginRequest expects field name `password` — NOT `userPassword`.
  # (RegisterRequest uses `userPassword`; the two DTOs differ on purpose.
  # Earlier versions of this seeder copy-pasted the register field name and
  # every login silently returned HTTP 400 "Request body validation failed".)
  resp=$(curl -sS -X POST "${AUTH_BASE_URL}/auth/login" \
    -H "Content-Type: application/json" \
    -d "{\"userName\":\"$username\",\"password\":\"$DEMO_PASSWORD\"}" 2>/dev/null) || resp=""
  [[ "$AUTH_SLEEP" -gt 0 ]] && sleep "$AUTH_SLEEP"
...
echo "═══════════════════════════════════════════════════════════════════"
echo ""
echo "  Sign in at https://anirudhhomes.in/login"
