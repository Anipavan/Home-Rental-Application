package com.spa.home_rental_application.document_service.ExceptionHandler;

import com.spa.home_rental_application.document_service.Exceptionclass.DTO.APIErrorResponse;
...
public class HandleException {
    ...
    public ResponseEntity<APIErrorResponse> handleNotFound(DocumentNotFoundException ex, HttpServletRequest req) {
        log.warn("Document not found at {}: {}", req.getRequestURI(), ex.getMessage());
        return build(HttpStatus.NOT_FOUND, ex.getMessage(), ex.getErrorCode(), req);
    }

    @ExceptionHandler(InvalidDocumentException.class)
    public ResponseEntity<APIErrorResponse> handleInvalid(InvalidDocumentException ex, HttpServletRequest req) {
        return build(HttpStatus.BAD_REQUEST, ex.getMessage(), ex.getErrorCode(), req);
    }

    @ExceptionHandler(InvalidPreSignedUrlException.class)
    public ResponseEntity<APIErrorResponse> handlePreSigned(InvalidPreSignedUrlException ex, HttpServletRequest req) {
        log.warn("Invalid pre-signed URL at {}: {}", req.getRequestURI(), ex.getMessage());
    ...
    public ResponseEntity<APIErrorResponse> handleStorage(StorageException ex, HttpServletRequest req) {
        log.error("Storage failure at {}: {}", req.getRequestURI(), ex.getMessage(), ex);
